...
##############################################################################################################
def bkg_test_plotter(param_combos, baseDir=baseDir, testDir='depths/COSMOS_test/'):
    """
    ...
    # get all filenames in dir and verify test parameters are present
    ...
    # TODO: show original image instead of seg
    ...
    # display bgmap, bgsub for both filters

    # --- Find first group with matching files for J and JH ---
    selected_group = None
    for (bksz, bkfsz), bands in grouped.items():
        if all(ftype in bands["J"] for ftype in ["bgmap", "bgsub", "seg"]) and \
           all(ftype in bands["JH"] for ftype in ["bgmap", "bgsub", "seg"]):
            selected_group = (bksz, bkfsz)
            break

    if not selected_group:
        raise ValueError("No group found with complete J and JH bgmap, bgsub, and seg images.")
