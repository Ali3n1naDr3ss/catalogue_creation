...
# Note: changed to my config_files dir in data/HSC_SSP_DR3
    ...
    #os.environ['EXTRACTOR_DIR'] = '/mnt/users/videouser/sextractor/share/sextractor' # Note from Rohan: I need to run sextractor differently.
    # updated for tinsley
    ...
    if os.path.isfile(seCatalogue) and overwrite == False:
        ...
        #os.system('/usr/local/shared/sextractor/2.25.0/bin/sex '+  imageName +' -c ' + inputSex + extraKeywords + keywords + keywordsWeight)
...
def sizemag_stars(inputCat, fiveSig, filterName, field, pixScale, plotDir = 'plots/', starDir = '', sizeType = 'FWHM_IMAGE', extra= '', plotName = 'sizemag.pdf'):
    ...
    if filterName == 'u' or filterName == 'HSC-R':
        classstarcut = 0.5
    else:
        classstarcut = 0.95
...
def bzk_stars(inputCatalogue, bfilt = 'B', zfilt = 'Z', kfilt = 'K', brange = [18.0, 25.0], zrange = [19.0, 25.0], krange = [17.0, 24.5], radiusCut = [0.0001, 2.5], interceptDelta = [0.0, 0.0]):
