...
def image_finder(reqFilters, size_arcsec, centre_ra, centre_dec, dataDir, cutoutDir='cutouts/', cutoutFile='cutoutNames.txt', verbose=True, overwrite=False):
    """ 
    Created the correct file names for image and weight files based on the required filters, cutout size, and position.
    """

    cutoutDir = os.path.join(dataDir, cutoutDir)
    cutoutFile = os.path.join(cutoutDir, cutoutFile)
    ...
    if size_arcsec != 'full_size':
        # create dir/file to store cutout files and their filenames
        if not os.path.isfile(cutoutFile):
            ...
            # file is empty, so call make_cutouts
            cutouts = make_cutout(reqFilters, size_arcsec, centre_ra, centre_dec, dataDir, cutoutDir, verbose=False, overwrite=overwrite)

        elif os.path.isfile(cutoutFile):
            # check is the desired cutouts already exist by checking saved filenames
            with open(cutoutFile, 'r') as c:
                if os.stat(cutoutFile).st_size == 0: # if file is empty, call make_cutouts
                    cutouts = make_cutout(reqFilters, size_arcsec, centre_ra, centre_dec, dataDir, cutoutDir, verbose=False, overwrite=overwrite)
                for line in c:
                    line = line.strip()
                    for filt in reqFilters:
                        ...
                        HSCcutToMatch = f"{cutoutDir}{filt}_DR3_{centre_ra_str}_{centre_dec_str}_size{size_arcsec}.fits"
                        HSCwhtToMatch = f"{cutoutDir}{filt}_DR3_wht_{centre_ra_str}_{centre_dec_str}_size{size_arcsec}.fits"
                        ...
                        if HSCcutToMatch not in cutoutsToMatch:
                            cutoutsToMatch.append(HSCcutToMatch)
                        if HSCwhtToMatch not in cutoutsToMatch:
                            cutoutsToMatch.append(HSCwhtToMatch)
                        ...
                        if re.fullmatch(pattern, line):
                            cutouts.append(line)
                        elif re.fullmatch(HSCpattern, line):
                            cutouts.append(line)
                        whtpattern = (
                            rf"{cutoutDir}"
                        ...
                        if re.fullmatch(whtpattern, line):
                            cutouts.append(line)
                        elif re.fullmatch(HSCwhtpattern, line):
                            cutouts.append(line)

        return cutouts, cutoutsToMatch
...
def make_cutout(filters, size_arcsec, centre_ra, centre_dec, dataDir, cutoutDir=baseDir+'data/COSMOS/cutouts/', verbose=True, overwrite=True):
    ...
    INPUT(s)
        filters(list[str])  Used to find the image and weight file for UVISTA/HSC filters of interest
    ...
    for filt in filters:
        origFilePaths.append((filt, dataDir + f'UVISTA_{filt}_DR6.fits'))
        origFilePaths.append((filt, dataDir + f'{filt}_DR3.fits'))
        origFilePaths.append((filt, dataDir + f'UVISTA_{filt}_DR6_wht.fits'))
        origFilePaths.append((filt, dataDir + f'{filt}_DR3_wht.fits'))

    for filt, file_ in origFilePaths:
        if not os.path.isfile(file_):
            ...
            #if uvist in imapge paths
            
            if 'UVISTA' in file_:
                outPrefix = cutoutDir + 'UVISTA_'
                outSuffix = f'_DR6_{ra_filename}_{dec_filename}_size{size_arcsec}.fits'
            elif 'HSC' in file_:
                outPrefix = cutoutDir
                outSuffix = f'_DR3_{ra_filename}_{dec_filename}_size{size_arcsec}.fits'
            else:
                print("WARNING: Don't recognise file: ", file_)

            cutoutPath = outPrefix + filt + outSuffix

            if  '_wht' and 'UVISTA' in fits.open(file_).filename():
                outWhtSuffix = '_DR6_wht_' + ra_filename +'_'+ dec_filename +'_'+ 'size' + str(size_arcsec) + '.fits'

            elif  '_wht' and 'HSC' in fits.open(file_).filename():
...
def bg_plotter(imagePaths='none', bgMapPaths='none', listBgSubDicts='none',
               badMags='none', segPaths='none', whtPaths='none',
               show_bad_mags=False, saveFig=True, overwrite=True,
               show_fig=False, verbose=True):
    """
    Makes figure showing data, bg map, bg subtracted, and seg map.
    """
    #TODO - not showing data or show_bad_mags?? its bc the dirs are broken in the bit wheere (cutoputpaths) append catPath
    import math
    import matplotlib
    matplotlib.use("TkAgg")  # or "QtAgg"
    import matplotlib.pyplot as plt
    ...
    figTitleSubstr = []
    for bgSubDict in listBgSubDicts:
        telescope = bgSubDict['Path'].split('/')[-1].split('_')[0]
        filterName = bgSubDict['Path'].split('/')[-1].split('_')[1][0:1]
        #filterName = telescope + '_' + filterName
        if telescope not in figTitleSubstr:
            figTitleSubstr.append(telescope)
        if filterName not in figTitleSubstr:
            figTitleSubstr.append(filterName)

        back_size = bgSubDict['back_size']
        back_filtersize = bgSubDict['back_filtersize']

        if back_size not in figTitleSubstr:
            figTitleSubstr.append(back_size)
        if back_filtersize not in figTitleSubstr:
            figTitleSubstr.append(back_filtersize)

        if 'cutout' in bgSubDict['Path']:
            cutoutsize = bgSubDict['Path'].split('/')[-1].split('_')[1][1:]
            cutoutsize = cutoutsize + 'as_'
            if cutoutsize not in figTitleSubstr:
                figTitleSubstr.append(cutoutsize)

    figTitleSubstr = "_".join(map(str, figTitleSubstr))
    print("title test:", figTitleSubstr)

    rows_per_fig = 3
    n_batches = math.ceil(len(listBgSubDicts) / rows_per_fig)

    print("\n>>>>>>>>> Making background comparison figures. Started at:",
    ...
    for batch_idx in range(n_batches):
        ...
        image_chunk = [] if imagePaths == 'none' else imagePaths[start:start+nrows]
        bgMap_chunk = [] if bgMapPaths == 'none' else bgMapPaths[start:start+nrows]
        seg_chunk   = [] if segPaths == 'none' else segPaths[start:start+nrows]
        wht_chunk   = [] if whtPaths == 'none' else whtPaths[start:start+nrows]

        fig, axes = plt.subplots(nrows=nrows, ncols=5, figsize=(20, 4*nrows))
        axes = np.atleast_2d(axes).reshape(-1, 5)
        ...
        for i in range(nrows):
            ...
            # --- image cutout ---
            if imagePaths != 'none' and i < len(image_chunk):
            ...
            # --- bgmap ---
            if bgMapPaths != 'none' and i < len(bgMap_chunk):
                bgMapPath = bgMap_chunk[i]
                bgMapPlotTitle = os.path.basename(bgMapPath).replace('.fits', '').replace('_cutout_', '')
                bgMapData, _, _ = get_fits_data(bgMapPath, verbose=verbose)
                vmin, vmax = zscale.get_limits(bgMapData)
                ax = axes[i, col_idx]
                ax.imshow(bgMapData, cmap='gray', origin='lower', vmin=vmin, vmax=vmax)
                ax.set_title(bgMapPlotTitle, fontsize=10)
                ax.xaxis.set_major_formatter(formatter)
                ax.axis('off')
                col_idx += 1
            ...
            if listBgSubDicts != 'none':
                ...
                bgSubData, _, _ = get_fits_data(bgSubPath, verbose=verbose)
                vmin, vmax = zscale.get_limits(bgSubData)
                ax = axes[i, col_idx]
                ax.imshow(bgSubData, cmap='gray', origin='lower', vmin=vmin, vmax=vmax)
                ax.set_title(bgSubPlotTitle, fontsize=10)
                ax.xaxis.set_major_formatter(formatter)
                ax.axis('off')
                col_idx += 1

            # --- segmap ---
            if segPaths != 'none' and i < len(seg_chunk):
                segPath = seg_chunk[i]
                segPlotTitle = os.path.basename(segPath).replace('.fits', '').replace('_cutout_', '')
                segData, _, _ = get_fits_data(segPath, verbose=verbose)
                vmin, vmax = zscale.get_limits(segData)
                ax = axes[i, col_idx]
                ax.imshow(segData, cmap='gray', origin='lower', vmin=vmin, vmax=vmax)
                ax.set_title(segPlotTitle, fontsize=10)
                ax.xaxis.set_major_formatter(formatter)
                ax.axis('off')
                col_idx += 1

            # --- weight map ---
            if whtPaths != 'none' and i < len(wht_chunk):
                whtPath = wht_chunk[i]
                whtPlotTitle = os.path.basename(whtPath).replace('.fits', '').replace('_cutout_', '')
                whtData, _, _ = get_fits_data(whtPath, verbose=verbose)
                vmin, vmax = zscale.get_limits(whtData)
                ax = axes[i, col_idx]
                ax.imshow(whtData, cmap='gray', origin='lower', vmin=vmin, vmax=vmax)
                ax.set_title(whtPlotTitle, fontsize=10)
                ax.xaxis.set_major_formatter(formatter)
                ax.axis('off')
                col_idx += 1
        ...
        if saveFig:
            ...
            outputPath = outDir + figTitleSubstr + f"_batch_{batch_idx}.png"
            try:
    ...
    if show_fig:
        print("\n>>>>>>>> Opening last background figure at:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        plt.show()

def bg_plotter1(imagePaths='none', bgMapPaths='none', listBgSubDicts='none', badMags='none', segPaths='none', whtPaths='none', show_bad_mags=False, saveFig=True, overwrite=True, show_fig=False, verbose=True):
    """
    ...
    for batch_idx in range(n_batches):
        ...
        # slice chunks for this batch (all aligned to listBgSubDicts)
        bgSub_chunk = listBgSubDicts[start:end]
        nrows = len(bgSub_chunk)
        ...
        for i in range(nrows):  # safe because each chunk has nrows elements
            col_idx = 0
            # plot cutout region
            if imagePaths != 'none' and i < len(image_chunk):
                if image_chunk:  # only if not empty
                    imagePath = image_chunk[i]
                    imagePlotTitle = os.path.basename(imagePath).replace('.fits', '')
                    imageData, _, _ = get_fits_data(imagePath, verbose=verbose)
                    vmin, vmax = zscale.get_limits(imageData)
    ...
    if saveFig and overwrite:
        outDir = '/raid/scratch/hullyott/cataloguing/DepthsTestDir/depths/plots/'
        outputPath = outDir + f"test_batch{batch_idx}.png"
        try:
            plt.savefig(outputPath, format='png', dpi=300)
            print(f"Figure saved as {outputPath}")
            plt.close()
    ...
    if saveFig and overwrite==True:
        outDir = '/raid/scratch/hullyott/cataloguing/DepthsTestDir/depths/plots/'
        outputPath = outDir +  'test2' + bgSubParams.replace(":", '').replace(' ', '_') + ".pdf"
        try:
            plt.savefig(outputPath, format='png', dpi=300)
            print(f"Figure saved as {outputPath}")
...
def grid_depths(gridTable, x, y, faster=True, verbose=False, nearby=False, n_jobs=-1):
    """
...
def image_depth(imagePath, zeropoint, cutouts=[], size='none', back_size=32, back_filtersize=9, apDiametersAS=np.array([1.8, 2.0, 3.0, 4.0, 5.0]), whtPath='NONE', whtType='NONE', IRACapDiametersAS=np.array([2.8, 3.8, 5.8, 9.8, 11.6]), segPath='NONE', outputDir='none', filterName='NONE', numApertures=300, step=200, overwrite=False, inputSex=baseDir+'data/bertin_config/video_mine.sex', strips=False, bgSub=True, mask='none', gridSepAS=3.0):
    ...
    """
    print("for the love of fuck", whtPath)
    # this dir is poo fix later

    #imageDir = os.path.join(baseDir, 'data/COSMOS/')
    #imagePath = os.path.join(imageDir, os.path.basename(imagePath))
    #whtPath = os.path.join(imageDir, os.path.basename(whtPath))
    ...
    if outputDir == 'none' and cutouts == []:
        outputDir = os.path.join(baseDir, 'depths/COSMOS/')
        if os.path.isdir(outputDir) == False:
            os.system('mkdir ' + outputDir)
        print("outputDir will be: ", outputDir)

    elif outputDir == 'none' and cutouts != []:
        outputDir = os.path.join(baseDir, 'depths/COSMOS/cutouts/')
        if os.path.isdir(outputDir) == False:
            os.system('mkdir ' + outputDir)
        print("outputDir will be: ", outputDir)
...
def _process_filter(fi, filterName, imagedata, fieldName, cutouts, size, back_size, back_filtersize, outputDir, overwrite, gridSepAS, apDiametersAS, queue, dataDir, reqFilters):
    """Helper function to process one filter in parallel.

        imagedata   contents of images.lis
    """
    imageName = imagedata['Image'][fi]
    whtName = imagedata['Weight'][fi]
    whtType = imagedata['Wht_type'][fi]
    zeropoint = imagedata['zeropoint'][fi]
    imageDir = imagedata['directory'][fi] # dir of base data image

    maskName = os.path.join(dataDir, f"/masks/{fieldName}/{imagedata['mask'][fi]}")

    if not cutouts == []:
        for cutout in set(cutouts):
            cutoutName = os.path.basename(cutout)
            if cutoutName.startswith(imageName.split('.')[0][:4]):
                imageName = cutoutName
            if cutoutName.startswith(whtName.split('.')[0][:4]):
                whtName = cutoutName
    
    if imageDir == 'here' and cutouts == []:
        imageDir = baseDir + 'data/'  # ensure consistent baseDir
    elif imageDir == 'here' and cutouts != []:
        imageDir = baseDir + 'data/COSMOS/cutouts/' 

    if queue == 'none':
        strips = True
        if fieldName == 'COSMOS':
            stripFilt = np.array(reqFilters)
            jj = (filterName == stripFilt)
            if np.any(jj):
                strips = True
        print("for the love of god", whtName)
        bgMapPath, bgSubDict, segPath = image_depth(
            imageDir+imageName, zeropoint, cutouts=cutouts, size=size,
            back_size=back_size, back_filtersize=back_filtersize,
            whtPath=imageDir+whtName, whtType=whtType,
            outputDir=outputDir, strips=strips, filterName=filterName,
            overwrite=overwrite, mask=maskName, gridSepAS=gridSepAS,
            apDiametersAS=apDiametersAS
        )

        print("Files saved to: ", outputDir)
        return bgMapPath, bgSubDict, segPath
    else:
        #  queue mode left serial, because it spawns addqueue jobs anyway
        return None, None, None

def get_depths(fieldName, fullsizeimages='none', cutouts='none', size='none', back_size=32, back_filtersize=9, queue='none', reqFilters=['all'], apDiametersAS=np.array([1.8, 2.0, 3.0, 4.0, 5.0]), dataDir=baseDir+'data/', outputDir='none', overwrite=False, ra_str='none', dec_str='none', verbose=True, saveFig=True, n_jobs=-1):
    """
    ...
    # PARALLEL LOOP
    results = Parallel(n_jobs=n_jobs)(delayed(_process_filter)(fi, filterName, imagedata, fieldName, cutouts, size, back_size, back_filtersize, outputDir, overwrite, gridSepAS, apDiametersAS, queue, dataDir, reqFilters)         for fi, filterName in enumerate(availableFilters))

    # unpack results
    bgMapPaths, listBgSubDicts, segPaths = [], [], []
    for bgMapPath, bgSubDict, segPath in results:
        if bgMapPath is not None:  # queue branch may return None
    ...
    for fi, filterName in enumerate(availableFilters):
        ...
        imageName = imagedata['Image'][fi]       # e.g. HSC_G_DR3.fits
        whtName   = imagedata['Weight'][fi]      # e.g. HSC_G_DR3_wht.fits
        whtType   = imagedata['Wht_type'][fi]
        zeropoint = imagedata['zeropoint'][fi]
        imageDir  = imagedata['directory'][fi]
        maskName  = f"/raid/scratch/data/masks/{fieldName}/{imagedata['mask'][fi]}"

        if cutouts != 'none':
            for cutout in set(cutouts):  # unique items in list
                cutoutName = os.path.basename(cutout)

                # --- IMAGE cutouts (science images only, not weights) ---
                if "wht" not in cutoutName:
                    imgBase = os.path.splitext(imageName)[0]  # strip .fits
                    if cutoutName.startswith(imgBase):
                        imageName = cutoutName
                        print("The imageName is", imageName)

                # --- WEIGHT cutouts (explicitly require 'wht' in name) ---
                if "wht" in cutoutName:
                    whtBase = os.path.splitext(whtName)[0]
                    if cutoutName.startswith(whtBase):
                        whtName = cutoutName
                        print("The whtName is", whtName)
        ...
        if queue == 'none':
            ...
            if fieldName == 'COSMOS':
                ...
                if np.any(jj):
                    strips = True 
            print("is this possible", whtName, imageName)
        ...
        if cutouts != 'none' and fullsizeimages == 'none':
            for filt in reqFilters:
                ...
                if re.fullmatch(subpattern, subsetPath):
                    ...
                    HSCcutoutPath = f"{baseDir}data/COSMOS/cutouts/{filt}_DR3_{ra_str}_{dec_str}_size{size}.fits" # corresponds to the catalogue and subset catalogue
    ...
    if cutouts != []:
        #print("what:", cutouts, cutoutPaths, catPaths, subsetPaths, badMags)
        ...
        bg_plotter(cutoutPaths, bgMapPaths=bgMapPaths, listBgSubDicts=listBgSubDicts, badMags=badMags, segPaths=segPaths, whtPaths=whtPaths, show_bad_mags=True, saveFig=saveFig, overwrite=saveFig, verbose=verbose)
