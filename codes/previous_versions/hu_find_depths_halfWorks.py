...
from hu_depth_codes import image_finder, make_cutout, get_depths
...
reqFilters = ['Y']#, 'J', 'H', 'K', 'JH', 'HK'] #
#reqFilters = ['HSC_G', 'HSC_R', 'HSC_I', 'HSC_Z', 'HSC_Y']# <<<< may want to change 'Y', 'J', 'H', 'K', 'JH', 'HK', 
back_size = 64
back_filtersize = 9 
size_arcsec = 10 #'full_size' # <<<<<<<<<< may want to change 
...
apDiametersAS = np.array([1.8]) # <<<<<<<<<< TODO: do other aps when code works consistently
########## Function call #######################################################
print("hu_find_depths.py run at:", datetime.now().strftime("%d-%m-%Y %H:%M:%S"))

if size_arcsec == 'full_size':
   imagePaths = image_finder(reqFilters, 'full_size', dataDir)

if size_arcsec != 'full_size':
    cutouts, cutoutsToMatch = image_finder(reqFilters, size_arcsec, centre_ra, centre_dec, dataDir)
    print("lost",cutouts, cutoutsToMatch)
...
for ff, fieldName in enumerate(fields):
    ...
    if size_arcsec != 'full_size':
        ...
        get_depths(fieldName, cutouts=cutouts, size=str(size_arcsec), back_size=back_size, back_filtersize=back_filtersize, queue='none', reqFilters=reqFilters, overwrite=overwrite, outputDir=dataDir, dataDir=dataDir, apDiametersAS=np.array([1.0, 1.8, 2.0, 3.0, 4.0]), ra_str=centre_ra_str, dec_str=centre_dec_str, verbose=verbose, saveFig=saveFig)

    elif size_arcsec == 'full_size':
        get_depths(fieldName, fullsizeimages=imagePaths, size=str(size_arcsec), queue='none', reqFilters=reqFilters, dataDir=dataDir, overwrite=overwrite, outputDir='none', apDiametersAS=np.array([1.0, 1.8, 2.0, 3.0, 4.0]), ra_str=centre_ra_str, dec_str=centre_dec_str, verbose=verbose, saveFig=saveFig, n_jobs=len(reqFilters))
