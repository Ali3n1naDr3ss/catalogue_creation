...
def find_matched_dets(maxsep_as, matchedDetsPath, testing=True, verbose=False):
    ...
    matchedCoords_dict = {} # result of initial cross-matching between PAIRS of filters i.e. Y-J matches, Y-H matches
    for df1, df2 in combinations(detectionFilters, 2):  # compare all unique pairs
        ...
        matchPair_key = df1 + df2
        matchedCoords_dict[matchPair_key] = ({df1: matchedCoords_df1}, {df2: matchedCoords_df2})
        #TODO:  for each matched pair, find matches with next single filter  (or pair?)
