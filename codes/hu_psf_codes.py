...
def get_psf(fieldName, queue='none', reqFilters=['all'],
            apDiametersAS=np.array([1.8, 2.0, 3.0, 4.0, 5.0]),
            dataDir=baseDir + 'data/', outputDir='none',
            overwrite=False, starsOnly=False, useCat='NONE'):
    ...
    def process_filter(fi, filterName):
        ...
        if queue == 'none':
            print(f"[{filterName}] Running here at {datetime.now().strftime('%H:%M:%S')}")
            print("outpitdir is:", outputDir)
            psfex(
                imageDir + imageName, filterName, fieldName, zeropoint, depthDir,
                whtName=imageDir + whtName, whtType=whtType,
                outputDir=outputDir, strips=strips,
                overwrite=overwrite, starsOnly=starsOnly,
                overwritePSF=False, useCat=useCat
            )
        else:
            print(f"[{filterName}] Spawning in the queue {queue}")
            tmpName = f"tmp_{fieldName}_{filterName}.sh"
            with open(tmpName, 'w') as f:
                f.write('#!/bin/bash\n')
                f.write(
                    f'python3 stupid_psf.py {imageDir + imageName} {imageDir + whtName} {whtType} '
                    f'{zeropoint} {outputDir} {strips} {filterName} {overwrite} {starsOnly} {fieldName} {depthDir}'
                )
            os.system(f"chmod +x {tmpName}")
            command = f"addqueue -c 'tmp_{queue}' -m 8 -q {queue} -d ./{tmpName}"
            os.system(command)

    # Run in parallel if no queue, otherwise fall back to queue system
    if queue == 'none':
        Parallel(n_jobs=len(reqFilters))(
            delayed(process_filter)(fi, filterName)
            for fi, filterName in enumerate(availableFilters)
        )
    else:
        for fi, filterName in enumerate(availableFilters):
            process_filter(fi, filterName)
...
def sizemag_stars(inputCat, fiveSig, filterName, field, pixScale, plotDir = 'plots/', starDir = '', sizeType = 'FWHM_IMAGE', extra= '', plotName = 'sizemag.pdf'):
    ...
    if  filterName == 'JH' or  filterName == 'HK':
