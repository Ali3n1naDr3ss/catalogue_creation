...
import numpy as np
from cat_codes_4fe769d_test import *
...
for ff, fieldName in enumerate(fields):
    ...
    for df, detectionFilt in enumerate(detectionFilters):
        if se:
            ...
            catDir = os.path.join(baseDir, 'data/catalogues/')

            if not os.path.isdir(catDir):
                os.system('mkdir ' + catDir)
            catDir = os.path.join(catDir, '{0}/'.format(fieldName))
            indiDir = catDir + 'det_{0}/'.format(detectionFilt)
    ...
    for df, detectionFilt in enumerate(detectionFilters):
        ...
        if mask:
            ...
            for ai, apD in enumerate(requiredCats):
                ...
                apply_mask(inputCat, fieldName, save = True, hsc=True, cutband='K')
