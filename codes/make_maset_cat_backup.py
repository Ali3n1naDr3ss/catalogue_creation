...
def match_objs(maxsep, testing=True, verbose=False):
    ...
    ax.set_xlim(min(coordsDict[df][1][nnDict[df]]) - maxsep_dimless, max(coordsDict[df][1][nnDict[df]]) + maxsep_dimless)
    ax.set_ylim(min(coordsDict[df][0][nnDict[df]]) - maxsep_dimless,max(coordsDict[df][0][nnDict[df]]).max() + maxsep_dimless)
    ax.set_xlim(1,3)
    ax.set_ylim(148,153)
    ...
    for i, df in enumerate(detectionFilters[:-1]):
        ...
        ax.scatter(coordsDict[df][1][nnDict[df]],
                   coordsDict[df][0][nnDict[df]],
                   marker='.', color=colour, label=df)
    ...
    if detectionFilters[i+1] in coordsDict:
        ...
        #ax.scatter(coordsDict[df][1][nnDict[df]],coordsDict[df][0][nnDict[df]],marker='.', color=colour)
        colourKey[df] = colour

    ax.set_xlabel("Dec (deg)")
    ax.set_ylabel("RA (deg)")
    plt.title(f"{k}th Nearest Neighbours")
    plt.legend()
    plt.show()
