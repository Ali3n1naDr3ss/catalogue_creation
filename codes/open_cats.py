...
def open_cats(cats, open_subset=True, overwrite=False):
    ...
    if open_subset == False:
        ...
        return subsets, badratiosDict

    return None, None

def call_open_cats(
            filters=['Y', 'J', 'H', 'K', 'JH', 'HK', 'HSC_G', 'HSC_R', 'HSC_I', 'HSC_Z', 'HSC_Y'],
            basecatDir='/raid/scratch/hullyott/cataloguing/DepthsTestDir/data/catalogues/COSMOS_full/det_',
            open_subset=False):

    cats = []

    for filt in filters:
        catDir = basecatDir + filt + '/'
    ...
    if len(cats) == len(filters):
        subsets, badratiosDict = open_cats(cats, open_subset=open_subset)
