...
def make_cutout(image_paths, size_arcsec=1000, ra='random', dec='random', corner='lower-left', output_path=None, verbose=True):
    ...
    def save_coordinates(directory, filename, ra, dec, verbose=True):
    ...
    for image in image_paths:
        if output_path is None:
            ...
            save_coordinates(randomCutoutDir, 'random_coords.txt', ra_filename, dec_filename)
...
def extract_filename(filename):
        ...
        det_cat_match = re.match(r"(\w+)bksz(\d+)bkfilt(\d+)_(\d+)_(\d+)", base)
