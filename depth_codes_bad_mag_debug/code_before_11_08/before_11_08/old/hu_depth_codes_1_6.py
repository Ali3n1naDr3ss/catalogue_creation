...
def get_depths(fieldName, back_size, back_filtersize, queue='none', reqFilters=['all'], apDiametersAS=np.array([1.8, 2.0, 3.0, 4.0, 5.0]), dataDir=baseDir+'data/', outputDir='none', overwrite=False):
    ...
    for fi, filterName in enumerate(availableFilters):
        ...
        if queue == 'none':
            ...
            outputCatalogue =  image_depth(imageDir + imageName, zeropoint, whtName = imageDir + whtName, whtType = whtType, outputDir = outputDir, strips = strips, filterName = filterName, overwrite = overwrite, mask = maskName, gridSepAS = gridSepAS, apDiametersAS = apDiametersAS, back_size=back_size, back_filtersize=back_filtersize)
