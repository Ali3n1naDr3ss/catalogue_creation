...
def detections_fig(cutout_filename, filters, detCats, subsetCriterion='none'):
    ...
    Makes a figure showing the science image, detections,
     and subset detections for two filters in a 3x2 formation. Subset creation happens here. 
        ...
        cutout_filename(str): /raid/scratch/data/COSMOS_test/random_cutouts/UVISTA_JH_DR6_15007186924767774_26432262759966996.fits"""
    ...
    for f, filt in enumerate(filters):
        ...
        # checks necessary files exist
        cutout_filename = os.path.basename(cutout_filename) # UVISTA_JH_DR6_15007186924767774_26432262759966996.fits
        prefix, suffix, _, ra, dec = extract_filename(cutout_filename)
        filename = f'{prefix}{filt}{suffix}{ra}_{dec}.fits'
        sciDir = dataDir + 'random_cutouts/'+ filename # testing on vista only
        ## science image
        ...
        for all detcats in dir, if ra, dec, filt match sciImg, extract data


        ## detections
        if os.path.isfile(detCats[f]):
            # detCats[f] == ../depths/COSMOS_test/catalogues/dJbksz32bkfilt9_1507797965685055_2217532084368842.fits
            print(f"Found detection catalogue: {detCats[f]} \n")
            prefix, suffix, _, ra_det, dec_det = extract_filename(detCats[f])
            if ra == ra_det and dec == dec_det:
                print(f"Matching cutout image and detection catalogue found: {ra}, {dec}")
            ...
            elif ra != ra_det or dec != dec_det:
                print(f">>>>>>>>>>>>>>>>>>>>> RA/DEC MISMATCH: \n Sci - {sciDir} \n det - {detCats[f]}")
                print("Attempting to find match...")
                detDir = detCats[f].split(os.path.basename(detCats[f]))[0]
        ...
        if os.path.isfile(subset_dir):
            print(f"Found subset catalogue: {subset_dir}")
            prefix, suffix, _, ra_sub, dec_sub = extract_filename(subset_dir)
            if ra == ra_sub and dec == dec_sub:
                print(f"Matching cutout image and subset found: {ra}, {dec}")
    ...
    for i, (sci_img, det_table, det_name, subset_table, subset_name) in enumerate(zip(sciImgs, detTables, detNames, subsetTables, subsetNames)):
        ...
        pre,suf,filt, ra, dec = extract_filename(sci_img)
        plot_name = "Science Image: " + filt
        ax.text(0, -30, f'RA/Dec:{ra[:7], dec[:7]}', fontsize=10, color='red')
    ...
    ## plot detections
        ...
        # plot
        filt, bksz, bkfilt, ra, dec = extract_filename(det_name)
        det_plot_name = det_name[1:]
        ax_det = axes[i, 1]
        ax_det.scatter(ra, dec, s=1, color='blue', alpha=0.5)
        ax_det.set_xlabel("RA (deg)")
        ax_det.set_ylabel("Dec (deg)")
        ax_det.set_title(f"All: {filt} BS:{bksz} BFILT:{bkfilt}")
    ...
    ## plot subset
        ...
        pre, suf, filt, ra, dec = extract_filename(subset_name)
        ax_sub.scatter(ra, dec, s=1, color='blue', alpha=0.5)
        ax_sub.set_xlabel("RA (deg)")
        ax_sub.set_ylabel("Dec (deg)")
        ax_sub.set_title(f"Subset: {filt} BS:{bksz} BFILT:{bkfilt}")
        ax_sub.text(0,-30, f'bad:all = {bad_ratios[i]:.5f}', fontsize=12, color='red')
